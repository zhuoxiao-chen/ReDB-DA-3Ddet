...
class DataAugmentor(object):
    ...
    def random_object_scaling(self, data_dict=None, config=None):
        ...
        if 'ONLY_FOR_SAMPLED_OBJECTS' in config.keys() \
                and config.ONLY_FOR_SAMPLED_OBJECTS:
            assert 'non_sampled_gt_boxes_num' in data_dict.keys()
            sampled_box_index = data_dict['non_sampled_gt_boxes_num']
        else:
            sampled_box_index = 0

        no_ROS_gt_boxes = data_dict['gt_boxes'][:sampled_box_index]

        points, gt_boxes = augmentor_utils.scale_pre_object(
            data_dict['gt_boxes'][sampled_box_index:], data_dict['points'],
            gt_boxes_mask=data_dict['gt_boxes_mask'][sampled_box_index:],
            gt_names = data_dict['gt_names'][sampled_box_index:],
            scale_perturb=config['SCALE_UNIFORM_NOISE']
        )

        data_dict['gt_boxes'] = np.concatenate((no_ROS_gt_boxes, gt_boxes))
        data_dict['points'] = points
        return data_dict
